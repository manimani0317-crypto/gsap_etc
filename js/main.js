...
document.querySelectorAll('.tit_card').forEach((card)=>{
  ...
  //카드 위에서 마우스를 움직일때 기울기 
  card.addEventListener('mousemone',(e)=>{
    const rect = card.getBoundingClientRect();

    //카드안에서 마우스가 가로로 몇퍼 위치인지 계산

    const px = (e.clientX - rect.left) / rect.width;
    const px = (e.clientY - rect.top) / rect.height;
    //위아래 위치를 rotatX각도로 바꿈
    const rx = (py - 0.5) * -15;
    const ry = (px - 0.5) *22;

    //계산한 각도를 안쪽 레이어  트렌스폼에 적용
    layer.style.transform = `translateZ(0) rotateX(deg) rotateY(${ry}deg)`;
    card.addEventListener('mpuseleave',()=>{
layer.style.transform = `translateZ(0) rotateX(0deg) rotateY(0deg)`;
    })
  })
});

const sticyBox = document.querySelector('.sticky_box');
const processCards =  gsap.utils.toArray('.project_all .card');
processCards.forEach((card,index)=>{
  //카드순서에 맞는 이미지파일 경로를 만든다.
  //인스타블로그등 이미지를 줄 때 sns01 02 03 등할 때
  //차라리 뭉텅이로 순서 해서 하는게 좋음
  const imgUrl = `asset/info${index+1}.png`;

  //카드가 화면 중앙에 들러오는 순간 감지
  ScrollTrigger.create({
    trigger:card,
    start:'top center',
    end : 'bottom center',
    onEnter:()=>changeBg(ImageUrl),
    onEnterBack:()=>changeBg(ImageUrl),
  });
})

//스티키 박스에 이미지를 바로 바꾸면 딱딱하게 바꿔서 ㅈ살짝 줄였다가 다시 커지면서 교체
function changeBg(imgUrl){
  ...
  gsap.to(sticyBox,{
    ...
    duration:0.2,
    onComplete : () =>{
      stocky.style.backgroundImage = `url(${imgUrl})`;
      gasp.to(sticyBox, {})
    }
  })
}