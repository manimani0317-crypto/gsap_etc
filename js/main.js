...
//가로 스크롤 섹션에서 실제로 옆으로 움직여야 되는 거리를 계산함
//트랙 전체 너비에서 화면에 이미보이는 너비를 빼면 이동거리만 남음
function totalWidth(){
...
const horizontalTween = gsap.to('.track',{
  //트랙을 왼쪽으로 이동 시켜 오른쪽 카드들이 보이게 한다.
  x: () => -totalWidth(),
  ease:'none',
  scrollTrigger:{
    trigger:'.horizontal_section',
    start: 'top top',
    //실제 이동거리보다 더 길게 잡아 앞 뒤 야백 스크롤을 만든다
    enf : () => '+=' +(totalWidth() + window.innerWidth * 1.45),
    scrub: true,
    pin:true,
...
// case_panel 텍스트는 수서대로, 이미지는 서로 다른 방향에서 등장
...
//그때 스크롤 트리거 위치 계산을 다시 맞추는 함수
function refreshScrollScenes(){
  //시작점과 끝점을 다시 계산하게 한다
ScrollTrigger.refresh(); 
//위치가 바뀐 뒤 액티브 메뉴도 다시 맞춤
updateActiveFromViewport() 
}
...
window.addEventListener('load', () => {
  ...
  //document.fonts)가 있는 브라우저하면 폰트 로딩도 기다린다
  if(document.fonts){
